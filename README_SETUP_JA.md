...
VoiceDesignCloner連携（Qwen3-TTS・簡易）は、多言語の声デザイン、声ガチャ、参照音声+文字起こしによる簡易クローン、選んだ声での簡易コーパス一括音声化、リサンプル、esd.list生成、Irodori-TTS LoRA学習データ準備、LoRA学習実行入口に対応する任意エンジンです。Voice-Design-ClonerのQwen3-TTSワークフローを参考に、このWeb UIから呼び出せるようにしています。
